...
Эта команда заменит конфигурацию по умолчанию. Обратите внимание на блоки ssl_certificate и ssl_certificate_key — тут нужно указать путь до файлов сертификатов, которые мы получили ранее с помощью Certbot. По умолчанию достаточно заменить youdomain.com на имя вашего домена.\
...
Допустим, у нас всего один домен. Создадим для него поддомен cdn.yourdomain.com и направим его на сервер-источник, создав A-запись.
Сам домен мы с помощью CNAME-записи направим на технический домен, который нам выдаст CDN-ресурс чуть позже. Этот домен мы запишем в поле «Адрес сервера» в клиенте.
